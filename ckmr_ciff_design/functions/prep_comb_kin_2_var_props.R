...
# -----------------------------------------------------
prep_comb_kin_2_var_props <- function(samp.no.py, years.samp, f_prop, lethal)
{
  library(dplyr)
  # PARS
  ...
  mf_ju_B  <- offarray(c(0, round(samp.no.py[, 'ju'] * f_prop)   ),    first = min(Bju_range), last = max(Bju_range))
  mf_sa_B  <- offarray(c(   round(samp.no.py[, 'subad'] * f_prop), 0), first = min(Bju_range), last = max(Bju_range))
  mf_ad_Y  <- offarray(c(   round(samp.no.py[, 'ad'] * f_prop)   ),    first = min(Yad_range), last = max(Yad_range))
  
  # mf_... is samp size. MALES
  mm_ju_B  <- offarray(c(0, round(samp.no.py[, 'ju'] * m_prop)   ), first=min(Bju_range), last=max(Bju_range))
  mm_sa_B  <- offarray(c(   round(samp.no.py[, 'subad'] * m_prop), 0), first=min(Bju_range), last=max(Bju_range))
  mm_ad_Y  <- offarray(c(   round(samp.no.py[, 'ad'] * m_prop)   ), first=min(Yad_range), last=max(Yad_range))
