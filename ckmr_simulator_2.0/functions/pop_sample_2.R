...
# -----------------------------------------------------
pop_sample_2 <- function(sam.no.py, years.samp, scen, gens, pop.samp.dead, repi, two_or_none)
{
  # PARS
  ...
  for (gen in seq(2, gens))
  {
    if (pop.samp.dead) 
    {
      population <- read.table(paste0("populations", two_or_none, "/mort_", scen, "/ciff_population_gen", gen, "_dead.txt"), 
                               sep = "\t", header = T)
    } else {
      population <- read.table(paste0("populations", two_or_none, "/mort_", scen, "/ciff_population_gen", gen, ".txt"), 
                               sep = "\t", header = T)
    }
    year          <- 1978 + gen
  ...
  write.table(pop.samp[-1, ], paste0("sampled_populations", two_or_none, "/mort_", scen, "/pop_samp_", repi, "_yrs_samp_",
