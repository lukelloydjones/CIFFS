...
# ----------------------------------------------------
find_kin_2 <- function(sam.no.py, years.samp, scen, repi, two_or_none)
{
  # PARS
  ...
  pop.samp <- read.table(paste0("sampled_populations", two_or_none, "/mort_", scen, "/pop_samp_", repi, "_yrs_samp_",
  ...
  # FSPS
  # ----
  check5 <- ((pop.samp.mlt$FP_P == pop.samp.mlt$FP_O) &  (pop.samp.mlt$MP_P == pop.samp.mlt$MP_O) & (pop.samp.mlt$inds.id_P != pop.samp.mlt$inds.id_O)) # Is indivdual i j's mother?
  fsps.inds  <- indices[check5, ]
  ...
  kin <- list(mops.inds, fops.inds, mhsps.inds, phsps.inds, fsps.inds)
  names(kin) <- c("mops", "fops", "mhsps", "phsps", "fsps")
  save(kin, file = paste0("found_kin", two_or_none, "/mort_", scen, "/pop_samp_", repi, "_yrs_samp_",
