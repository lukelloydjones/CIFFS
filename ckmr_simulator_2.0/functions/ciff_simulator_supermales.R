...
#  * Trying to add supermales
# =================================================================
ciff_simulate_supermales <- function(start.size, generations, mratepar, super_mort, type_no)
{
  # PARS
  ...
  dir.create(file.path(paste0("populations", type_no, "/mort_", mratepar)))
  # ------------------------------------
  # Initialise the supermale set
  # ------------------------------------
  ...
  # Supermales
  spr.mal.no <- floor(0.1 * length(breed.fem))
  super.males  <- breed.mal[sample(seq(1, length(breed.mal)), spr.mal.no)]
  ...
  for (gen in seq(2, generations))
  {
    ...
    # Supermale
    #  Dominant for a year and then with some probability someone can know 
    #  you off your perch. You getting back in is hard but possible
    #  0.1 supermales compared to females
    # ------------------------------
    adults    <- population[which(population$life.sg.status == "A"), ]
    a.males   <- adults[which(adults$mf == 1), ]
    a.females <- adults[which(adults$mf == 0), ]
    ...
    # Supermales
    spr.mal.no  <- floor(0.1 * length(breed.fem))
    #spr.mal.no.to.kepp  <- floor(0.5 * spr.mal.no) # 50% turn over rate
    spr.mal.no.to.kepp  <- floor((1-super_mort) * spr.mal.no) # super_mort% turn over rate
    # Some may have died so remove them
    super.males <- super.males[which(super.males %in% breed.mal)]
    if (spr.mal.no.to.kepp < length(super.males))
    {
      set.to.keep <- super.males[sample(seq(1, length(super.males)), spr.mal.no.to.kepp)]
    } else {
      set.to.keep <- super.males
    }
    # Draw from the potential set of new hot shots
    pot.set.new <- breed.mal[which(!breed.mal %in% super.males)]
    set.new     <- pot.set.new[sample(seq(1, length(pot.set.new)), (spr.mal.no - length(set.to.keep)))]
    # Concatenate to make the new set of champs
    super.males <- c(set.to.keep, set.new)
    # ---
    # Age
    # ---
    ...
    FP             <- breed.fem
    MP             <- sample(super.males, length(breed.fem), replace = T)
    year           <- year.i
    ...
    # -----------------------------
    write.table(dead, paste0("populations", type_no, "/mort_", mratepar, "/ciff_population_gen", gen, "_dead.txt"), 
                col.names = T,
                row.names = F, 
                quote = F, 
                sep = "\t")
    ...
    write.table(population, paste0("populations", type_no, "/mort_", mratepar, "/ciff_population_gen", gen, ".txt"), 
                col.names = T,
                row.names = F, 
                quote = F, 
                sep = "\t")
...
# Plot and write out
jpeg(filename = paste0("populations", type_no, "/mort_", mratepar, "/ciff_population_change.jpg"), 
     pointsize = 26, 
     quality = 1000, 
