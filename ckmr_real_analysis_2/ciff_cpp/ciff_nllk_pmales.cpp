...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(R1);                                  // - Female ROI
  PARAMETER(R2);                                  // - Male ROI
  ...
  fhsp_prob = fhsp_prob * fudge_factor;
  //std::cout << "MOP probabilities" << std::endl;
...
//  std::cout << "Birth years total" << byrs_tot<< std::endl;
  ...
  if (surv_prior)
  {
    std::cout << "HELLP SURVIVAL PRIOR" << std::endl;
    ciff_llk += pow(Z1 - Mu_Z1, 2.0) / (2.0 * pow(SD_Z1, 2.0));
    ciff_llk += pow(Z2 - Mu_Z2, 2.0) / (2.0 * pow(SD_Z2, 2.0));
  }
